...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, ScenarioGenerator)
...
# inputs
def Trend_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ScenarioGenerator)
        agent_object.TrendI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def Asset_Action_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ScenarioGenerator)
        agent_object.Asset_ActionI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def Generate_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ScenarioGenerator)

        agent_object.GenerateI = value

        if value:
            print("[ScenarioGenerator] Generate received → generating scenario")
            agent_object.generate_scenario()

    except:
        print(traceback.format_exc())


def Asset_Category_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ScenarioGenerator)
        agent_object.Asset_CategoryI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.definition_set_class("ScenarioGenerator")
    ...
    igs.input_create("Trend", igs.STRING_T, None)
    igs.observe_input("Trend", Trend_input_callback, agent)
    igs.input_create("Asset_Action", igs.STRING_T, None)
    igs.observe_input("Asset_Action", Asset_Action_input_callback, agent)
    igs.input_create("Generate", igs.BOOL_T, None)
    igs.observe_input("Generate", Generate_input_callback, agent)
    igs.input_create("Asset_Category", igs.STRING_T, None)
    igs.observe_input("Asset_Category", Asset_Category_input_callback, agent)
    igs.input_create("Asset_ID", igs.STRING_T, None)
    igs.observe_input("Asset_ID", Asset_Id_input_callback, agent)

    igs.output_create("Future_Prices", igs.DATA_T, None)
    igs.output_create("History_Prices", igs.DATA_T, None)
    igs.output_create("Scenario_Ready", igs.BOOL_T, None)
